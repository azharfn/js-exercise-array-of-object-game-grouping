function gameGrouping(games) {
    ...
    games.forEach((game) => {
        ...
        if(!GrupGame[genre]){
            GrupGame[genre] = {title : [title]};
        }
        else{
            GrupGame[genre].title.push(title);
        }
    });
    return GrupGame;
    // ///jika mencari berdasar rating 
    // const GrupGamebyRate = {
    //     'jikaRating<8': [],
    //     'jikaRating<10': [],
    //     'jikaRating=10': [],
    // };
    ...
    // games.forEach((game) => {
    //     const { rating, title, genre } = game;
    
    //     if (rating < 8) {
    //     GrupGamebyRate['jikaRating<8'].push({ rating, title, genre });
    //     } else if (rating < 10) {
    //     GrupGamebyRate['jikaRating<10'].push({ rating, title, genre });
    //     } else if (rating === 10) {
    //     GrupGamebyRate['jikaRating=10'].push({ rating, title, genre });
    //     }
    // });
    
    // return GrupGamebyRate;
